...
class CasinoPlayer(loggable.Loggable):
    """Class to represent the player/user."""

    def __init__(self, stats_init=None):
        self.addLogger()
        self._current_game = None  # No game is set initially
        ...
        if stats_init is not None:
            # Initialize cash and account balance
            self.cash = stats_init.pop("cash", prepare.MONEY)
            self.account_balance = stats_init.pop("account balance", 0)

            # Initialize per-game stats
            for game_name, game_stats in stats_init.items():
                self._stats[game_name] = OrderedDict(game_stats)

        # Fallback for games not in stats_init
        if "Craps" not in self._stats:
            self._stats["Craps"] = OrderedDict({
                'times as shooter': 0,
                'bets placed': 0,
                'bets won': 0,
                'bets lost': 0,
        ...
        self.current_game = "Craps"  # Default to Craps
