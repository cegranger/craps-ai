...
def take_picture(cap, padding = 50):
    ...
    dice, centroids = get_dice_from_blobs(blobs)

    crops = []
    for centroid in centroids:
        minX = int(max(centroid[0] - padding, 0))
        minY = int(max(centroid[1] - padding, 0))
        maxX = int(min(centroid[0] + padding, frame.shape[1]))
        maxY = int(min(centroid[1] + padding, frame.shape[0]))
        print('X', minX, maxX)
        print('Y', minY, maxY)
        crops.append(frame[minY:maxY, minX:maxX, ...])
    # while True:
